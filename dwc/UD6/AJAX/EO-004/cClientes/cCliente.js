...
class cCliente {
  ...
  constructor() {
    ...
    document.getElementById("CargaClientes").addEventListener("click", function (event) {
     ...
     document.getElementById("bID").addEventListener("change", function (event) {
      var IdBuscat=parseInt(document.getElementById("bID").value);
      var fila=0;
      var trobatID=false;
      //Ompli l'array amb tots els ind
      for(var i=0;i<aClientes.length;i++){
        if(IdBuscat==aClientes[i]['id']){
          fila=i;
          trobatID=true;          
          break;
        }
      ...
      if(trobatID){
        ...
        ctlCliente.SetVista(aClientes[fila]);
      }else{
        document.getElementById("bID").value="";
        alert("¡ Index no encontrado !");
      }      
        event.preventDefault;
    });
